...
class GlobalLocalAttention(nn.Module):
    def __init__(self,
        ...
        self.localx1= ConvBN(dim, dim, kernel_size=3,dilation=1)
        self.localx2 = ConvBN(dim, dim, kernel_size=3, dilation=2)
        self.localx3 = ConvBN(dim, dim, kernel_size=3, dilation=3)
    ...
    def forward(self, x):
        ...
        # 将输入张量的通道维度平均分为 3 份
        localx1, localx2, localx3 = torch.chunk(local, 3, dim=1)
...
class Decoder(nn.Module):
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        edge_preds = []
        print("!!!!res1.shape=",res1.shape)#res1.shape= torch.Size([4, 128, 128, 128])                                                                                                                               
        print("!!!!res2.shape=",res2.shape)#res2.shape= torch.Size([4, 256, 64, 64]) 
        print("!!!!res3.shape=",res3.shape)#res3.shape= torch.Size([4, 512, 32, 32]) 
        print("!!!!res4.shape=",res4.shape)#res4.shape= torch.Size([4, 1024, 16, 16])
        print("!!!!h=",h)#h=51
        print("!!!!w=",w)#w=1024

        #x = self.b4(self.pre_conv(res4))#
        x=self.ppm(res4)#torch.Size([4, 1024, 32, 32])
        # print("x1.shape=",x.shape)
        
        y=self.cnnstem4(res3)
        # print("y.shape=",y.shape)#y.shape= torch.Size([4, 1024, 32, 32])
        x=self.pp4(x,y)
        # print("x2.shape=",x.shape)#x2.shape= torch.Size([4, 1024, 32, 32])
        x = self.pre_conv(x)
        # print("x3.shape=",x.shape)#x3.shape= torch.Size([4, 256, 32, 32])
        x = self.b4(x)
        # print("x4.shape=",x.shape)#x4.shape= torch.Size([4, 256, 32, 32])
        x=self.pre_conv1(x)
        # print("x5.shape=",x.shape)#x5.shape= torch.Size([4, 512, 32, 32])
        x,edge_pred=self.pfm3([x,res3])
        # print("x6.shape=",x.shape)#x6.shape= torch.Size([4, 512, 64, 64])
        edge_preds.append(edge_pred)
        # print("edge_preds.shape=",edge_preds[0].shape)#[4,1,32,32]

        y=self.cnnstem3(res2)
        # print("y1.shape=",y.shape)#y1.shape= torch.Size([4, 512, 64, 64])
        x=self.pp3(x,y)
        # print("x7.shape=",x.shape)#x7.shape= torch.Size([4, 512, 64, 64])
        x = self.prea_conv(x)
        # print("x8.shape=",x.shape)#x8.shape= torch.Size([4,256, 64, 64])
        x = self.b3(x)
        # print("x9.shape=",x.shape)#x9.shape= torch.Size([4,256, 64, 64])
        # x=self.prea_conv1(x)
        # print("x10.shape=",x.shape)#x10.shape= torch.Size([4, 128, 64, 64])
        x,edge_pred=self.pfm2([x,res2])
        # print("x11.shape=",x.shape)#x11.shape= torch.Size([4, 128, 128, 128])
        edge_preds.append(edge_pred)

        y=self.cnnstem2(res1)
        # print("y2.shape=",y.shape)#y2.shape= torch.Size([4, 256, 128, 128])
        x=self.pp2(x,y)
        # print("x12.shape=",x.shape)#x12.shape= torch.Size([4, 256, 128, 128])
        x = self.b2(x)
        # print("x13.shape=",x.shape)#x13.shape= torch.Size([4, 256, 128, 128])
        x=self.prea_conv1(x)
        # print("x14.shape=",x.shape)#x14.shape= torch.Size([4, 128, 128, 128]
        x,edge_pred=self.pfm1([x,res1])
        # print("x15.shape=",x.shape)#x14.shape= torch.Size([4, 128,256, 256])
        edge_preds.append(edge_pred)

        y=self.cnnstem1a(res1)
        # print("y3.shape=",y.shape)#y3.shape= torch.Size([4, 128,128,128])
        x=self.pp1(x,y)
        # print("x16.shape=",x.shape)#x16.shape= torch.Size([4, 128, 128,128])
        ...
        # x = self.p3(x, res3)#输入通道数为512，输出通道数为256
        # x = self.b3(x)#输入通道数为256，输出通道数为256
        ...
        # x = self.p1(x, res1)
        x=self.pre_conv2(x)
        # print("x17.shape=",x.shape)#x17.shape= torch.Size([4, 256, 128,128])
        x = self.segmentation_head(x)
        # print("x18.shape=",x.shape)#x18.shape= torch.Size([4, 256, 128,128]))
...
def xyc_unetformer(pretrained=True, num_classes=6, freeze_stages=-1, decoder_channels=256,
    ...
    model = XYCUNetFormer(num_classes=num_classes,
