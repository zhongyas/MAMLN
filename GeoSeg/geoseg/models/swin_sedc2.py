...
class SE(nn.Module):
    ...
    def forward(self, x):
        ...
        x5 = 0
        x6 = self.maxpool(swin_input_1_refine)
        x7 = self.maxpool(swin_input_2_refine)
        x8 = self.maxpool(swin_input_3_refine)
...
class DCFAM(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, x7, x8):
        ...
        print(x8.shape)
        out4 = x8 + self.down34(self.down232(x2)) #[4, 768, 32, 32]

        # print('out4:', out4.shape)

        # print('out4:', out4.shape)#[4, 768, 32, 32]
        out3 = x7 + self.down231(self.down12(x1)) #[4, 384, 64, 64]
        # out3 = self.maxpool(self.maxpool(concat2))
        out2 = x6 + self.lf4(out4) #[4, 192, 128, 128]
...
class Decoder(nn.Module):
    ...
    def forward(self,x1, x2, x3, x4, x5, x6, x7, x8):
        out1, out2 = self.dcfam(x1, x2, x3, x4, x5, x6, x7, x8)
        x = out1 + self.up1(out2)
        x = self.dropout(x)
