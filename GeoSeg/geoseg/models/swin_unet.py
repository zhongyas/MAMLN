...
class UNet_emb(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_heads=(4, 8, 16, 32),
                 frozen_stages=2,
                 in_channels=3, out_channels=3, bias=False):
        """Initializes U-Net."""
        ...
        self.segmentation_head = nn.Sequential(
            ConvBNReLU(3, 3),
            Conv(3, num_classes, kernel_size=1))
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder
        swin_out_3 = self.conv1(swin_out[3])
        ...
        # print('in:', self._block6(concat1).shape)
        # out1 = self._block6(concat1)
        # out2 = self._block6(concat1)
        # result = torch.cat((out1,out2),dim=1)
        result = self.segmentation_head(self._block6(concat1))

        # print('out:', result.shape)


        # print(self._block6(concat1).shape)
        # Final activation
        return result
...
def bmnet_swin(pretrained=True, num_classes=6, weight_path='pretrain_weights/stseg_small.pth'):
    model = UNet_emb(encoder_channels=(96, 192, 384, 768),
