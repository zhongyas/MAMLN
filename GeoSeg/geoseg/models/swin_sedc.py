...
class SE(nn.Module):
    ...
    def forward(self, x):
        ...
        concat3 = torch.cat((swin_input_3, swin_input_3_refine, swin_input_3_refine), dim=1)  # 256+256+256==768
        ...
        concat2 = torch.cat((swin_input_2, swin_input_2_refine, swin_input_2_refine), dim=1)  # 128+128+128=384
        ...
        concat1 = torch.cat((swin_input_1, swin_input_1_refine, swin_input_1_refine), dim=1)  # 64+64+64=192
        decoder_1 = self.ReLU(self.conv3(concat1))  # 64
        upsample5 = self._block5(decoder_1)  # 32

        # print('in:', x.shape) #[4,3,1024,1024]
        x5 = 0
        x6 = self.maxpool(concat1)
...
class DCFAM(nn.Module):
    def __init__(self, encoder_channels=(96, 192, 384, 768), atrous_rates=(6, 12), num_classes=6, dropout=0.05, bias=False):
        ...
        # self.embedding_dim = 3
        # self.conv_0 = nn.Conv2d(3, self.embedding_dim, 3, stride=1, padding=1)
        # self.conv_1 = nn.Conv2d(768, 768, 3, stride=2, padding=1)
        # self.conv_2 = nn.Conv2d(384, 384, 3, stride=2, padding=1)
        # self.conv_3 = nn.Conv2d(192, 192, 3, stride=2, padding=1)
        # self.conv_4 = nn.Conv2d(96, 96, 3, stride=2, padding=1)
        # self.in_chans = 3
        ...
        #
        # self._block6 = nn.Sequential(
        #     nn.Conv2d(96 + 3 + self.embedding_dim, 64, 3, stride=1, padding=1),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(64, 32, 3, stride=1, padding=1),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(32, 3, 3, stride=1, padding=1),
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, x7, x8):
        ...
        # print('x8:', x8.shape)
        out4 = self.conv_4(x4) + self.down34(self.down232(x2)) + self.cla(x8) #[4, 768, 32, 32]

        # print('out4:', out4.shape)

        # print('out4:', out4.shape)#[4, 768, 32, 32]
        out3 = self.conv_3(x3) + self.down231(self.down12(x1)) + self.cla(x7) #[4, 384, 64, 64]
        # out3 = self.maxpool(self.maxpool(concat2))
        out2 = self.conv_2(x2) + self.lf4(out4) + self.cla(x6) #[4, 192, 128, 128]
        # out2 = self.maxpool(self.maxpool(concat1))

        out1 = self.conv_1(x1) + self.lf3(out3)#[4, 96, 256, 256]
