...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.b1_2=SLBlock(dim=decode_channels,num_heads=16, window_size=window_size)
        self.b2_2=SLBlock(dim=decode_channels,num_heads=16, window_size=window_size)
        self.b3_2=SLBlock(dim=decode_channels,num_heads=16, window_size=window_size)
        self.b4_2=SLBlock(dim=decode_channels,num_heads=16, window_size=window_size)
        ...
        # self.res_up_1_0=ConvBN(encoder_channels[0],decode_channels,kernel_size=1)
        # print("encoder_channels[0]",encoder_channels[0])
        # print("encoder_channels[1]",encoder_channels[1])
        self.res_down_0_1=ConvBN(encoder_channels[0], decode_channels, kernel_size=1)
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        ...
        # print('interpolate', x)
        '''
        res4_1=self.b4_2(self.pre_conv(res4))
        
        res3_w1=self.res3_w1_relu(self.res3_w1)
        weight=res3_w1/(torch.sum(res3_w1,dim=0)+self.epsilon)
        # print("res3.shape",res3.shape)
        # print("res4_1.shape",res4_1.shape)
        res3_up=self.b3_2(weight[0]*self.res_down_2_3(res3)+weight[1]*self.res_upsample(res4_1))
        
        res2_w1=self.res2_w1_relu(self.res2_w1)
        weight=res2_w1/(torch.sum(res2_w1,dim=0)+self.epsilon)
        res2_up=self.b2_2(weight[0]*self.res_down_1_2(res2)+weight[1]*self.res_upsample(res3_up))
        
        
        res1_w1=self.res1_w1_relu(self.res1_w1)
        weight=res1_w1/(torch.sum(res1_w1,dim=0)+self.epsilon)
        # print(res1.shape)
        # print(self.res_upsample(res2_up).shape)
        res1_out=self.b1_2(weight[0]*self.res_down_0_1(res1)+weight[1]*self.res_upsample(res2_up))
        # res1_out=self.res_up_1_0(res1_out)
        ...
        # # print("res2_out.shape",res2_out.shape)
        # # print("res1_out.shape",res1_out.shape)
        # # print("self.res_down_sample(res1_out)",self.res_down_sample(res1_out).shape)
        # # res2_out=self.res_down_1_2(res2_out)
        ...
        # # res3_out=self.res_down_2_3(res3_out)
        ...
        # # print("res3_out.shape",res3_out.shape)
        # # print("self.res_down_sample(res3_out)",self.res_down_sample(res3_out).shape)
        # # print("self.res_down_3_4(res4)",self.res_down_3_4(res4).shape)
        ...
        x = self.segmentation_head(res1_out)
        x = F.interpolate(x, size=(h, w), mode='bilinear', align_corners=False)
