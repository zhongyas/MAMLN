...
class affinityattention(nn.Module):
    def __init__(self):
        super().__init__()
        self.extra_linear_e = nn.Linear(256, 256,bias=False)
        self.channelattention=ChannelAttention(256) 
    def forward(self, feature1, feature2):
        fea_size1 = feature1.size()[2:]#获取特征图的大小
        all_dim1= fea_size1[0]*fea_size1[1]#计算特征图的总像素点数
        
        fea_size2 = feature2.size()[2:]
        all_dim2= fea_size2[0]*fea_size2[1]

        feature1_flat=feature1.view(-1, feature2.size()[1], all_dim1)#维度是[batch_size,256,all_dim1]
        feature2_flat=feature2.view(-1, feature2.size()[1], all_dim2)
        
        feature1_t = torch.transpose(feature1_flat,1,2).contiguous()#维度是[batch_size,all_dim1,256]
        feature1_corr = self.extra_linear_e(feature1_t)#维度是[batch_size,all_dim1,256]

        feature2_t = torch.transpose(feature2_flat,1,2).contiguous()#维度是[batch_size,all_dim2,256]
        feature2_corr = self.extra_linear_e(feature2_t)#维度是[batch_size,all_dim2,256]

        A2 = torch.bmm(feature1_corr, feature2_flat)#维度是[batch_size,all_dim1,all_dim2]
        
        A = F.softmax(A2, dim = 1)
        #逐行选择最大值
        A_max,A_max_index=torch.max(A,dim=1)
        # print("A_max:",A_max)
        # print("A_max_index:",A_max_index)
        feature1ca=self.channelattention(feature1)
        feature1view=feature1ca.view(-1, feature1ca.size()[1], feature1ca.size()[2]*feature1ca.size()[3])#维度是[batch_size,256,all_dim1]
        #增加一维度
        A_max_view=A_max.unsqueeze(1)
        # print("feature1view.shape",feature1view.shape)
        # print("A_max_view:",A_max_view.shape)
        input1_att=torch.bmm(feature1view,A_max_view)
        ...
        input1_att = input1_att.view(-1, input1_att.size()[1], fea_size1[0], fea_size1[1])  
        return input1_att
...
class aandbattention(nn.Module):
    def __init__(self):
        super().__init__()
        self.channelat=ChannelAttention(256)
    def forward(self, feature1, feature2):
        ...
        feature1_att = torch.bmm(torch.transpose(A,1,2).contiguous(),A_max_view).contiguous()#
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.aandbattention=aandbattention()
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        ...
        #y1和xconcat
        y1=self.aandbattention(y1.softmax(dim=1),x.softmax(dim=1))
        # print("y1:",y1.size())
        ...
        y2=self.aandbattention(y2.softmax(dim=1),x.softmax(dim=1))
        ...
        y3=self.aandbattention(y3.softmax(dim=1),x.softmax(dim=1))
        ...
        y4=self.aandbattention(y4.softmax(dim=1),x.softmax(dim=1))
