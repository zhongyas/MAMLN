...
class Decoder(nn.Module):
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        ...
        y1=self.coaffinityat(F.relu(y1),F.relu(x))
        # print("y1:",y1.size())
        ...
        y2=torch.cat([F.relu(y2),F.relu(x)],dim=1)
        #使用1*1卷积使通道减半
        y2=self.conv1a1(y2)
        ...
        y3=torch.cat([F.relu(y3),F.relu(x)],dim=1)
        #使用1*1卷积使通道减半
        y3=self.conv1a1(y3)
        ...
        y4=torch.cat([F.relu(y4),F.relu(x)],dim=1)
        #使用1*1卷积使通道减半
        y4=self.conv1a1(y4)
