...
class DCFAM(nn.Module):
    ...
    def forward(self, x, x1, x2, x3, x4):
        ...
        out4 = self.conv_4(x4) + self.down34(self.pa(self.down232(x2))) + self.maxpool(self.maxpool(concat3)) #[4, 768, 32, 32]
        # print('out4:', out4.shape)

        # print('out4:', out4.shape)#[4, 768, 32, 32]
        out3 = self.pa(x3) + self.down231(self.ca(self.down12(x1))) + self.maxpool(self.maxpool(concat2)) #[4, 384, 64, 64]

        out2 = self.ca(x2) + self.lf4(out4) + self.maxpool(self.maxpool(concat1))#[4, 192, 128, 128]

        del out4
        out1 = self.lf3(out3) + self.conv_1(x1) #[4, 96, 256, 256]
...
class Decoder(nn.Module):
    ...
    def forward(self,x, x1, x2, x3, x4):
        out1, out2 = self.dcfam(x, x1, x2, x3, x4)
        x = out1 + self.up(out2)
        x = self.dropout(x)
