...
class PatchPartition(nn.Module):
    ...
    def forward(self, img):
        ...
        print("patchimg.shape: ", img.shape)
        x = self.partition(img)
        print("partition.shape: ", x.shape)
        _, H, W, _ = x.shape
        print("partition1.shape: ", x.shape)
        return x, H, W


class LinearEmbedding(nn.Module):
    def __init__(self, patch_dim, embed_dim=128, norm_layer=None):
...
class ToPatchEmbed(nn.Module):
    def __init__(self, patch_height, patch_width):
        ...
        self.linear_embedding = LinearEmbedding(patch_dim, embed_dim=128)
...
class SwinUnet(nn.Module):
    def __init__(self, embed_dim, patch_height, patch_width, class_num):
        ...
        self.basic_layer_0_e = BasicLayer(dim=embed_dim,
                                        depth=2,
                                        num_heads=4,
                                        window_size=8,
                                        mlp_ratio=4,
        ...
        self.basic_layer_1_e = BasicLayer(dim=2 * embed_dim,
                                        depth=2,
                                        num_heads=8,
                                        window_size=8,
                                        mlp_ratio=4,
                                        ...
                                        downsample=None,
                                        use_checkpoint=False)
        self.basic_layer_2_e = BasicLayer(dim=4 * embed_dim,
                                        depth=18,
                                        num_heads=16,
                                        window_size=8,
                                        mlp_ratio=4,
                                        ...
                                        downsample=None,
                                        use_checkpoint=False)
        self.basic_layer_single_e = BasicLayer(dim=8 * embed_dim,
                                             depth=2,
                                             num_heads=32,
                                             window_size=8,
                                             mlp_ratio=4,
                                             ...
                                             use_checkpoint=False)
        #decoder
        self.basic_layer_0_d = BasicLayer(dim=embed_dim,
                                        depth=2,
                                        num_heads=4,
                                        window_size=8,
                                        mlp_ratio=4,
                                        ...
                                        downsample=None,
                                        use_checkpoint=False)
        self.basic_layer_1_d = BasicLayer(dim=2 * embed_dim,
                                        depth=2,
                                        num_heads=8,
                                        window_size=8,
                                        mlp_ratio=4,
                                        ...
                                        downsample=None,
                                        use_checkpoint=False)
        self.basic_layer_2_d = BasicLayer(dim=4 * embed_dim,
                                        depth=2,
                                        num_heads=16,
                                        window_size=8,
                                        mlp_ratio=4,
                                        ...
                                        downsample=None,
                                        use_checkpoint=False)
        self.basic_layer_single_d = BasicLayer(dim=8 * embed_dim,
                                             depth=1,
                                             num_heads=32,
                                             window_size=8,
                                             mlp_ratio=4,
    ...
    def forward(self, img):
        ...
        print("ximg:", x.shape)
        x1, H, W = self.basic_layer_0_e(x, H, W)
        '''x1=1,3136,96'''
        # step2
        x, H, W = self.patch_merging_0(x1, H, W)  # patchmerging从上到下编号
        x2, H, W = self.basic_layer_1_e(x, H, W)
        # step3
        x, H, W = self.patch_merging_1(x2, H, W)
        x3, H, W = self.basic_layer_2_e(x, H, W)
        x, H, W = self.patch_merging_2(x3, H, W)

        '''BottleNeck'''
        x, H, W = self.basic_layer_single_e(x, H, W)  # step1
        y, H, W = self.basic_layer_single_d(x, H, W)  # step2
...
def ft_unetformer(pretrained=True, classes_num=6, freeze_stages=-1, decoder_channels=256,
                  weight_path='pretrain_weights/stseg_base.pth'):
    model = SwinUnet(class_num=classes_num,
                         embed_dim=128,
                         patch_height=4,
                         patch_width=4)
...
if __name__ == '__main__':
    ...
    swin_unet = SwinUnet(embed_dim=128,
                         patch_height=4,
                         patch_width=4,
                         class_num=100)
