...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x): # x 的输入格式是：[batch_size, C, H, W]
        ...
        out = avg_out + max_out#维度是[batch_size,256,1,1]
...
class coaffinityattention(nn.Module):
    ...
    def forward(self, feature1, feature2):
        ...
        #使用通道注意力机制
        feature1ca=self.channelat(feature1)#维度是[batch_size,256,1,1]
        # print("feature1ca1",feature1ca.size())
        ...
        # print("feature1ca2",feature1ca.size())
...
class Decoder(nn.Module):
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        if self.training:
            ...
            ah=res1_out+self.up1(res2_out)+self.up2(res3_out)+self.up3(res4_out)
            ah = self.dropout(ah)
...
def ft_unetformer_small_nopre(pretrained=None, num_classes=6, freeze_stages=-1, decoder_channels=256,
                  weight_path='pretrain_weights/stseg_small.pth'):
    model = FTUNetFormer(num_classes=num_classes,
                         freeze_stages=freeze_stages,
                         embed_dim=96,
